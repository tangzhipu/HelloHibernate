...
public class HQLTest {
	...
	static{
		...
		//加载配置文件
		configuration.configure("ouc/jeep/hql/hibernate.cfg.xml");
	...
	/**
	 * 利用带构造函数的select查询结构更好，在持久化类中必须提供该构造函数，
	 * 		注意别默认的构造函数
	 */
	@Test
	public void testClasses_Select_Constructor(){
		Session session = sessionFactory.openSession();
		List classesList = session.createQuery("select new ouc.jeep.hql.Classes(cid,cname) from Classes").list();
		classesList.size();
		session.close();
	}
	...
	public void testSelect_Constructor_Classes_Student_Inner(){
		Session session = sessionFactory.openSession();
		List classesList = session.createQuery("select new ouc.jeep.hql.ClassesView(c.cname,s.sname) " +
...
//			4、用迫切内连接把classes和student和course三张表的数据联系在一起，如果有重复的数据去掉重复
	 ...
	 * classesList.size();
	 * session.createQuery("from d").iterate()
	 * 有什么区别
	 *   从 发出的语句 怎么不同
	 * 	   利用缓存的不同?
	 */
