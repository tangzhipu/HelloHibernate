package ouc.jeep.associationMapping.manyToMany;
...
/**
  *多对多的体现在类中就是类中的集合
      	增加关系体现的就是集合增加了一条数据
      	解除关系体现的就是集合中删除了一条数据
      	一般情况下遍历集合的时候用一般的for循环
  *谁维护关系要看谁的映射文件中的invser属性的值
 */

/**
 * 一对多和多对多的关系的区别：
   * 一对多最好是多的一方维护关系，这样效率高，避免发出update语句
   * 多对多哪一方维护关系都行
   * 关系的体现
       多对多的关系体现就是集合
       一对多
...
public class ManyToManyTest {
	...
	static{
		...
		configuration.configure("ouc/jeep/associationMapping/manytomany/hibernate.cfg.xml");
	...
	public void testSaveCourse(){
		...
		course.setCname("001班");
		course.setDescription("001");
	...
	public void testSaveCourse_Cascade(){
		 ...
		 * 通过课程建立课程与学生之间的关系
		 */
		course.setStudents(students);//因为课程是一个新的，所以根本没有学生
		session.save(course);
	 ...
	 * 		Hibernate: select course0_.cid as cid0_0_, course0_.cname as cname0_0_, course0_.description as descript3_0_0_ from Course course0_ where course0_.cid=?
			Hibernate: select max(sid) from Student
			Hibernate: insert into Student (sname, description, sid) values (?, ?, ?)
			Hibernate: insert into student_course (sid, cid) values (?, ?) 该sql语句是维护关系
	 */
	@Test
