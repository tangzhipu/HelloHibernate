...
public class OneToManyTest {
	...
	static{
		...
		configuration.configure("ouc/jeep/associationMapping/oneToMany/hibernate.cfg.xml");
	...
	public void testSaveClassAndStudent(){
		...
		Student student = new Student();
		student.setSname("小李");
		student.setDescription("副版主");
		session.save(student);
		
		Classes classes = new Classes();
		classes.setCname("002班");
		classes.setDescription("002");
		session.save(classes);
		transaction.commit();
		session.close();
	}
	...
	public void testSaveClass_Cascade_Student(){
		...
		session.save(student); // 显示保存：只要session.save操作了一个对象，就是显示保存
		
		Set<Student> students = new HashSet<Student>();
		students.add(student);
		Classes classes = new Classes();
		classes.setCname("003班");
		classes.setDescription("003");
	 ...
	 */
	/** 
	 *    如果没有写cascade属性，在保存classes的时候，student仍然是临时状态,
	 *  	  在两个对象关联的时候，进行保存的时候，hibernate不允许有临时状态的对象的出现
	 */
	@Test
	public void testSaveClassesAndStudent_R(){
		...
		classes.setStudents(students); //隐式保存：无session.save(student)，不在程序中做保存的动作，而交给hibernate来做
		session.save(classes);
	...
	public void testSaveClasses_R(){
		...
		Classes classes = new Classes();
		classes.setCname("004班");
		classes.setDescription("004");
		Set<Student> students = new HashSet<Student>();
		students.add(student);
		classes.setStudents(students);
	...
	public void testSaveClasses_R_2(){
		...
		Classes classes = new Classes();
		classes.setCname("005班");
		classes.setDescription("005");
		Set<Student> students = new HashSet<Student>();
		students.add(student);
		classes.setStudents(students);
	...
	public void testRealse_Rebuild(){
		...
		Classes classes2 = (Classes)session.get(Classes.class, 9L);
	...
	public void testRealseAll_Rebuild_R(){
		...
		Classes classes = (Classes)session.get(Classes.class, 5L);
		List<Student> students = session.createQuery("from Student where sid in(1,3)").list();
		//重新建立班级和学生之间的关系
