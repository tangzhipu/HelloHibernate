package ouc.jeep.session.cache;
...
import ouc.jeep.associationMapping.oneToMany.Bidirectional.Classes;
import ouc.jeep.associationMapping.oneToMany.Bidirectional.Student;
...
public class OneCacheTest {
	...
	public void testFlush(){
		 ...
		 *   *  如果该对象是持久化对象，并且该对象的ID在数据库中有对应的记录，并且该对象的属性有变化，
		 *   	则会自动发出update语句，如果该对象的属性没有变化，则不发出update语句
		 *   *  检查持久化对象中是否有关联的对象，如果有关联的对象，并且设置了级联操作，
		 *   	这个时候会检查级联对象的id在数据库中有没有对应的记录，如果有，则发出update语句，如果没有，则发出insert语句
		 *   *  如果有维护关系的代码，则还会改变关系
	...
	public void testFlush2(){
		...
		student1.getClasses().setCname("0001");
		student2.getClasses().setDescription("00002");
		session.flush();
		transaction.commit();
	}
